...
describe('searchEventsIndex', () => {
  it('finds events matching a single filter', () => {
    const matches = searchEventsIndex(SAMPLE_EVENTS, ['jazz'])
    expect(matches.has('Jazz Night at The Blue Room')).toBe(true)
    expect(matches.has('Jazz Brunch')).toBe(true)
    expect(matches.has('Trivia Tuesday')).toBe(false)
  })
  ...
  it('finds events matching multiple filters', () => {
    ...
    expect(matches.has('Jazz Night at The Blue Room')).toBe(true)
    expect(matches.has('Jazz Brunch')).toBe(true)
    expect(matches.has('Trivia Tuesday')).toBe(true)
    expect(matches.has('Farmers Market')).toBe(false)
  })
  ...
  it('matches on location field', () => {
    ...
    expect(matches.has('Farmers Market')).toBe(true)
  })

  it('matches on description field', () => {
    const matches = searchEventsIndex(SAMPLE_EVENTS, ['stand-up comedy'])
    expect(matches.has('Comedy Open Mic')).toBe(true)
  })
})

describe('extractMatchingVEvents', () => {
  it('extracts VEVENTs with matching summaries', () => {
    const summaries = new Set(['Jazz Night at The Blue Room', 'Jazz Brunch'])
    const blocks = extractMatchingVEvents(SAMPLE_ALL_ICS, summaries)
  ...
  it('returns empty array when no summaries match', () => {
    const summaries = new Set(['Nonexistent Event'])
    const blocks = extractMatchingVEvents(SAMPLE_ALL_ICS, summaries)
    expect(blocks.length).toBe(0)
  })

  it('returns all events when all summaries match', () => {
  ...
  it('each returned block is a valid VEVENT', () => {
    const summaries = new Set(['Trivia Tuesday'])
    const blocks = extractMatchingVEvents(SAMPLE_ALL_ICS, summaries)
