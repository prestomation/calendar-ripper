...
import { RipperConfig, toICS, externalConfigSchema, ExternalConfig, ExternalCalendar, RipperCalendar } from "./config/schema.js";
...
const generateCalendarList = (ripper: RipperConfig, outputs: CalendarOutput[], calendarTags: Map<string, string[]>) => {
    // Get ripper-level tags
    const ripperTagsArray = ripper.tags || [];
    const ripperTagsHtml = ripperTagsArray.length > 0 
        ? `<div class="ripper-tags">${ripperTagsArray.map(tag => `<span class="tag">${tag}</span>`).join(' ')}</div>` 
        : '';
    ...
    const toc = outputs.map(calendar => {
        ...
        const calendarKey = `${ripper.name}-${calendar.icsPath.split('-')[1].split('.')[0]}`;
        const calendarTagsArray = calendarTags.get(calendarKey) || [];
        const calendarTagsHtml = calendarTagsArray.length > 0 
            ? `<div class="calendar-tags">${calendarTagsArray.map(tag => `<span class="tag">${tag}</span>`).join(' ')}</div>` 
            : '';
...
const generateExternalCalendarList = (externals: ExternalCalendar[]) => {
    ...
    const toc = externals.map(calendar => {
        ...
        const tagsHtml = tagsArray.length > 0 
            ? `<div class="calendar-tags">${tagsArray.map(tag => `<span class="tag">${tag}</span>`).join(' ')}</div>` 
            : '';
        
        let entry = `<div class="calendar-item">
            <div class="calendar-title">
                <a href="${calendar.icsUrl}">${calendar.friendlyname}</a>
                ${calendar.infoUrl ? `(<a href="${calendar.infoUrl}">info</a>)` : ''}
            </div>
            ${tagsHtml}
            <div class="calendar-actions">
                <a href="${webcalLink}" title="Subscribe to this calendar in iCal/Outlook">[Subscribe]</a>
                <button class="copy-btn" data-clipboard-text="${calendar.icsUrl}" title="Copy calendar URL to clipboard">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                        <path d="M4 1.5H3a2 2 0 0 0-2 2V14a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V3.5a2 2 0 0 0-2-2h-1v1h1a1 1 0 0 1 1 1V14a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V3.5a1 1 0 0 1 1-1h1v-1z"/>
                        <path d="M9.5 1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5v-1a.5.5 0 0 1 .5-.5h3zm-3-1A1.5 1.5 0 0 0 5 1.5v1A1.5 1.5 0 0 0 6.5 4h3A1.5 1.5 0 0 0 11 2.5v-1A1.5 1.5 0 0 0 9.5 0h-3z"/>
                    </svg>
...
export const main = async () => {
    ...
    for (const config of configs) {
        ...
        for (const calConfig of config.config.calendars) {
            if (calConfig.tags && calConfig.tags.length > 0) {
                calendarTags.set(`${config.config.name}-${calConfig.name}`, calConfig.tags);
            }
        }
        ...
        tableOfContents += generateCalendarList(config.config, outputs, calendarTags);
    };
    ...
    const taggedCalendars = prepareTaggedCalendars(allCalendars, ripperTags, calendarTags);
    const taggedExternalCalendars = prepareTaggedExternalCalendars(activeExternalCalendars);
    
    // Debug logging
    console.log("Tagged Calendars:", taggedCalendars.map(tc => `${tc.calendar.friendlyname} (${tc.tags.join(', ')})`));
    console.log("Tagged External Calendars:", taggedExternalCalendars.map(tec => `${tec.calendar.friendlyname} (${tec.tags.join(', ')})`));
    ...
    if (aggregateCalendars.length > 0) {
        ...
        tableOfContents += generateAggregateCalendarList(aggregateOutputs, taggedCalendars, taggedExternalCalendars);
    }
...
const generateAggregateCalendarList = (outputs: CalendarOutput[], taggedCalendars: TaggedCalendar[], taggedExternalCalendars: TaggedExternalCalendar[]) => {
    if (outputs.length === 0) {
        return "";
    }
    ...
    const toc = outputs.map(calendar => {
        ...
        const sourceCalendars = taggedCalendars
            .filter(tc => tc.tags.some(tag => tag.toLowerCase() === tagName.toLowerCase()))
        ...
        const sourceExternalCalendars = taggedExternalCalendars
            .filter(tec => tec.tags.some(tag => tag.toLowerCase() === tagName.toLowerCase()))
        ...
        const sourcesHtml = allSources.length > 0
            ? `<div class="aggregate-sources">
                ...
                ${allSources.map(source => `<div class="aggregate-source">${source}</div>`).join('')}
              </div>`
            : '';
