...
- **Report skipped items as errors** — new custom rippers must push a `ParseError` to the `errors` array when a parse method returns `null` (e.g., regex didn't match, missing required field, unparseable date). Silent skips hide broken parsers. See AGENTS.md "Skipped Items Must Be Reported as Errors" for the required pattern. Exceptions: deduplication skips, intentional content filters, and items that don't match the expected type (e.g., non-event products) do not need error reporting.