...
describe('EventbriteRipper (Elliott Bay)', () => {
    it('parses all events from sample data', () => {
        const ripper = new EventbriteRipper();
        const data = loadSampleData();
        const events = ripper.parseEvents(data.events, tz, DEFAULT_LOCATION);
    ...
    it('uses default 2-hour duration when end time is missing', () => {
        const ripper = new EventbriteRipper();
        const noEndEvents = [
            {
                id: 'test-no-end',
                name: { text: 'No End Time Event' },
                description: null,
        ...
        const events = ripper.parseEvents(noEndEvents, tz, DEFAULT_LOCATION);
    ...
    it('falls back to defaultLocation when venue is null', () => {
        ...
        const events = ripper.parseEvents(noVenueEvents, tz, DEFAULT_LOCATION);

        const valid = events.filter(e => 'summary' in e) as RipperCalendarEvent[];
        expect(valid[0].location).toBe(DEFAULT_LOCATION);
    });
    ...
    it('deduplicates events with the same ID', () => {
        ...
        const events = ripper.parseEvents(doubled, tz, DEFAULT_LOCATION);
    ...
    it('handles malformed event gracefully', () => {
        ...
        const events = ripper.parseEvents(malformed, tz, DEFAULT_LOCATION);
    ...
    it('uses unknown as context for events with no ID', () => {
        const ripper = new EventbriteRipper();
        const noIdEvents = [
            { id: undefined, name: null, start: { local: '2026-03-10T19:00:00', timezone: 'America/Los_Angeles' }, end: null, venue: null, url: null, description: null }
        ];
        const events = ripper.parseEvents(noIdEvents, tz, DEFAULT_LOCATION);
